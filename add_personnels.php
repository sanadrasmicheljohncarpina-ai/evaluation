...
$stmt = $mysqli->prepare(
    "INSERT INTO users (full_name, username, role, department, designation, photo, is_active, source, created_at)
     VALUES (?, ?, ?, ?, ?, ?, 1, 'admin_nologin', NOW())"
);
$stmt->bind_param("ssssss", $full_name, $username, $role, $department, $designation, $photo_file);
    if ($stmt->execute()) {
...
$department = $mysqli->real_escape_string(trim($_POST['department'] ?? ''));
    ...
    if ($photo_file !== null) {
        $stmt = $mysqli->prepare("UPDATE users SET full_name=?, role=?, department=?, designation=?, photo=? WHERE id=? AND source='admin_nologin'");
        $stmt->bind_param("sssssi", $full_name, $role, $department, $designation, $photo_file, $uid);
            } else {
$stmt = $mysqli->prepare("UPDATE users SET full_name=?, role=?, department=?, designation=? WHERE id=? AND source='admin_nologin'");
$stmt->bind_param("ssssi", $full_name, $role, $department, $designation, $uid);
    }
    $stmt->execute();
    $stmt->close();
...
<div class="modal">
    ...
    <input
        ...
        id="department"
        name="department"
        placeholder="e.g. Registrar's Office, College of Information Technology"
...
<div class="modal">
    ...
    <form method="POST" action="add_personnels.php?sector=<?= urlencode($viewSector) ?>" enctype="multipart/form-data">
        ...
        <div class="form-group">
    <label for="edit_department">Department / Office</label>
    <input
        type="text"
        id="edit_department"
        name="department"
